...
import sys
from app import db
from app.auth.models import Users, UserLocations
...
def display_locations(user):
    ...
    locations = UserLocations.query.filter_by(user_id=user.id).order_by(UserLocations.location_name).all()
...
def add_location(user):
    ...
    existing = UserLocations.query.filter_by(
        user_id=user.id, 
