...
# TODO YELLOW: outdated, use tags instead!
...
def display_categories(user):
    ...
    categories = (
        UserCategories.query.filter_by(user_id=user.id)
        .order_by(UserCategories.category_name)
        .all()
    )
...
def add_category(user):
    ...
    if not confirm_action(
        f"Add category '{category_name}' for user '{user.display_name}'?"
    ):
    ...
    # Create category
    try:
        category = UserCategories(user_id=user.id, category_name=category_name)
