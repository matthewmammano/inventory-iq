...
def validate_squad_access(squad: str) -> str | None:
    ...
    if not squad:
        logger.info("Squad access rejected: missing squad", extra={"squad": None})
        flash("Squad name is required.", "warning")
        return url_for("auth.login")
    if current_user.is_authenticated and current_user.display_name == squad:
        if not current_user.active:
            logger.warning("Squad access rejected: inactive current user squad", extra={"agency_id": current_user.id, "squad": squad})
    ...
    if not agency:
        logger.warning("Squad access rejected: unknown squad", extra={"squad": squad})
...
def validate_admin_session(squad: str, timeout_seconds: int = ADMIN_TIMEOUT) -> str | None:
    ...
    if not session.get("admin"):
        ...
        logger.info("Admin session rejected: missing", extra={"squad": squad})
    ...
    if not last_active or now - last_active > timeout_seconds:
        session.pop("admin", None)
        session.pop("admin_last_active", None)
        logger.info("Admin session rejected: expired", extra={"squad": squad})
