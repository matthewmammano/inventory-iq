document.addEventListener("DOMContentLoaded", () => {
    ...
    const flash = window.InventoryFlash;
    ...
    form?.addEventListener("submit", (event) => {
        ...
        if (!fromRadio || !toRadio) {
            ...
            flash?.show("warning", "Choose both From and To locations before continuing.");
            return;
        }
        if (hasInvalidCombination()) {
            event.preventDefault();
            flash?.show("warning", "Choose a different To location before continuing.");
        }
    });
    updateInvalidState();
});
